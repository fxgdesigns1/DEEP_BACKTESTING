...
class DesktopBacktestingSystem:
    ...
    def __init__(self, config_file: str):
        ...
        logger.info("Desktop Backtesting System initialized")
        logger.info("System optimized for high-performance hardware")
        logger.info("Ready to process large datasets")
    ...
    def load_market_data(self, data_dir: str, instruments: List[str] = None) -> bool:
        ...
        try:
            ...
            for instrument in instruments:
                ...
                if not csv_files:
                    logger.warning(f"No data files found for {instrument}")
                    continue
            ...
            for instrument, df in self.data_cache.items():
                ...
                if 'timestamp' in df.columns:
                    ...
                    if not filtered_df.empty:
                        ...
                        logger.warning(f"No data for {instrument} in specified date range")
    ...
    def _simulate_strategy(self, strategy_id: str, strategy_config: Dict[str, Any],
        ...
        try:
            ...
            # Daily trade counter to enforce limits per instrument
            daily_trade_counts: Dict[str, Dict[str, int]] = {}
            ...
            for instrument, df in market_data.items():
                ...
                for idx, row in df.iterrows():
                    # Enforce max trades per day if configured
                    try:
                        max_trades_per_day = strategy_config.get('optimization', {}).get('max_trades_per_day')
                    except Exception:
                        max_trades_per_day = None
                    if max_trades_per_day:
                        day_key = pd.to_datetime(row['timestamp']).strftime('%Y-%m-%d')
                        if instrument not in daily_trade_counts:
                            daily_trade_counts[instrument] = {}
                        if day_key not in daily_trade_counts[instrument]:
                            daily_trade_counts[instrument][day_key] = 0
                        if daily_trade_counts[instrument][day_key] >= int(max_trades_per_day):
                            continue
                    # Generate trading signals
                    signal = self._generate_signal(strategy_id, instrument, df, idx, params)
                    
                    if signal:
                        # Gate by min_signal_strength if present
                        min_strength = params.get('min_signal_strength')
                        if min_strength is not None and signal.get('strength', 0.0) < float(min_strength):
                            continue
                        ...
                        if trade_result:
                            ...
                            if max_trades_per_day:
                                daily_trade_counts[instrument][day_key] += 1
    ...
    def _generate_signal(self, strategy_id: str, instrument: str, 
        ...
        try:
            ...
            if strategy_id == "alpha_strategy":
                ...
                # Calculate momentum
                momentum = self._calculate_momentum(price_history['mid_price'].values, params.get('momentum_period', 10))
                
                # Normalize momentum to [0,1] as a simple strength proxy
                strength = float(min(1.0, max(0.0, abs(momentum) * 10)))
    ...
    def _simulate_trade_execution(self, signal: Dict[str, Any], 
        ...
        try:
            ...
            # Apply spread if enabled with dynamic session/instrument factors
            entry_price = signal['entry_price']
            if config.include_spread:
                base_spread = float(market_data['ask'] - market_data['bid'])
                # Session factor based on hour of day
                hour = pd.to_datetime(market_data['timestamp']).hour if 'timestamp' in market_data else 12
                if 0 <= hour < 8:
                    session = 'asian'
                elif 8 <= hour < 13:
                    session = 'european'
                    ...
                    session = 'us'
                session_factor = self.config.get('spread_modeling', {}).get('session_factors', {}).get(session, 1.0)
                instrument_factor = self.config.get('spread_modeling', {}).get('instrument_factors', {}).get(signal['instrument'], 1.0)
                effective_spread = base_spread * float(session_factor) * float(instrument_factor)
                if signal['direction'] == 'BUY':
                    entry_price += effective_spread / 2
                else:
                    entry_price -= effective_spread / 2
            
            # Apply slippage if enabled (instrument-aware)
            if config.include_slippage:
                slip_cfg = self.config.get('slippage_simulation', {})
                base_pips_map = slip_cfg.get('base_slippage_pips', {})
                ...
                if 'JPY' in instrument:
                    ...
                    pip_value = 0.0001
                base_pips = float(base_pips_map.get(instrument, 0.1))
                vol_factor = float(slip_cfg.get('volatility_factor', 0.5))
                slippage = (base_pips * pip_value) * (1.0 + vol_factor)
                if signal['direction'] == 'BUY':
            ...
            # Simulate simple exit: attempt to hit TP/SL within lookahead horizon
            exit_price = None
            exit_time = None
            direction = signal['direction']
            tp = signal['take_profit']
            sl = signal['stop_loss']
            current_ts = pd.to_datetime(signal['timestamp'])
            instrument_df = self.data_cache.get(signal['instrument']) if hasattr(self, 'data_cache') else None
            if instrument_df is not None and 'timestamp' in instrument_df.columns:
                future = instrument_df[instrument_df['timestamp'] > current_ts].head(24)
                for _, r in future.iterrows():
                    if direction == 'BUY':
                        ref = r.get('bid', r.get('mid_price', None))
                        if ref is None:
                            continue
                        if ref >= tp:
                            exit_price = tp; exit_time = r['timestamp']; break
                        if ref <= sl:
                            exit_price = sl; exit_time = r['timestamp']; break
                    else:
                        ref = r.get('ask', r.get('mid_price', None))
                        if ref is None:
                            continue
                        if ref <= tp:
                            exit_price = tp; exit_time = r['timestamp']; break
            ...
            if exit_price is None:
                exit_price = tp
                exit_time = current_ts + timedelta(hours=2)
    ...
    def optimize_strategy(self, strategy_id: str, parameter_ranges: Dict[str, Tuple[float, float]],
        ...
        try:
            ...
            if optimization_method == "grid_search":
                ...
                logger.warning(f"Unsupported optimization method: {optimization_method}")
                result = None
...
def main():
    ...
    logger.info("Optimized for high-performance hardware (3080 GPU / 5950X CPU / 64GB RAM)")
